...
/*
 *  本函数会extract enricher感兴趣的name/value，调用该enricher的enrich函数。
 *  会将insertion数据结构中保存新字符串的buf的指针传入，这样避免enricher自己去申请内存
 *  函数执行完后获得了所有insertion，包括个数，原始串中要被替换的字符串的起始和结束位置
 *  用来替换的字符串及长度。这些信息将被传给生成最终结果的函数
 */
